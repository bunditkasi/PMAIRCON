...
export function OperationalTable({
  ...
  columns,
  rows,
  emptyMessage = "No operational follow-up items in the selected scope.",
                ...
                <tr key={`${title}-${index}`}>
                  {row.map((cell, cellIndex) => (
                    <td
                      ...
                      className="rounded-[1rem] border border-[var(--border)] bg-[var(--surface)] px-3 py-3 text-sm text-[var(--text)] align-top"
                    >
                      {cell}
                    </td>
                  ))}
